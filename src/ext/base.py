...
from merlin import Bot, Context
from modules import excepts
from modules.const import chk_sudo, is_sudoers
...
class Base(commands.Cog):
    ...
    Load this extension as an external file with `client.load_extension('cogs.base')`
    ---
    This cog contains:
    ## Commands
    - settings
    ...
    def __init__(self, bot):
        # ? https://gist.github.com/InterStella0/b78488fb28cadf279dfd3164b9f0cf96#3-modulardynamic
        self._original_help_command = bot.help_command
        bot.cmd_help = self.cmd_help
        bot.help_command = None
        # bot.help_command = self.cmd_help
    ...
    async def init_sets(self, guild: discord.Guild):
        ...
        try:
            ...
            settings[f"g{guild.id}"] = settings["default"].copy()
            return
        # fix cmdHdl
        cmdHdl = settings["default"]["cmdHdl"].copy()
        # the following code will leave entrys already
        cmdHdl.update(settings[f"g{guild.id}"]["cmdHdl"])
        # exists and add the missing entrys so that
        settings[f"g{guild.id}"]["cmdHdl"] = cmdHdl
        # overwriting can be prevented
        default = settings["default"].copy()
        default.update(settings[f"g{guild.id}"])
        # we can also do the same thing for the whole settings
    ...
    async def sett_proc(self, ctx, base, entry: str, val: str):
        # process settings assignments
        ...
        if isinstance(base[entry], list):  # process the []s
            if " " not in val or not val.startswith(
                ("add", "rm", "remove", "del")
            ):
                raise excepts.HaltInvoke(base[entry])
            op = val.split()[0]
            ...
            if op in ("rm", "remove", "delete", "del"):
                ...
                await self.sett_set_val(base[entry], val)
        # proess the {}s, feed back into itself
        if isinstance(base[entry], dict):
            if " " not in val:
                raise excepts.HaltInvoke(base[entry])
            entry_ = val.split()
            val_ = " ".join(val.split()[1:])
            # recursion
            return await self.sett_proc(ctx, base[entry], entry_, val_)
        try:
            base[entry] = int(val)
        except ValueError:
            base[entry] = val
    ...
    async def cmd_help(self, ctx, *, query: str = ""):
        ...
        if query.lower() == "global":
            em = discord.Embed(
                title="Command list", description="wd: `/`", color=0x0000FF
            )
            ...
            if cog_name.lower() == query.lower():
                ...
                for cmd in cog.walk_commands():
                    if any(cmd.parents) or " " in cmd.qualified_name:
                        continue
                    em.add_field(
                        name=cmd.name, value=cmd.short_doc or "<no help>"
                    )
                return await ctx.send(embed=em)
        # show help for command
        if query:
            command = await bot.get_command(query, i=ctx.message)
            if not command or command.hidden:
                return await ctx.send("help: :mag: Command not found.")
            path = f"/{command.cog.qualified_name if command.cog else 'GLOBAL'}/{command.full_parent_name.replace(' ', '/')}"
            em = discord.Embed(
            ...
            for val in command.clean_params.values():
                usage += f"<{val.name}>" if val.default else f"<[{val.name}]> "
            em.add_field(name="Objective", value=command.help)
            em.add_field(name="Usage", value=usage)
            em.add_field(
                name="Cog",
                value="<No cog>"
                if not command.cog
                else command.cog.qualified_name,
            )
            em.add_field(
                name="Aliases",
                value=", ".join(command.aliases) or "<No aliases>",
            )
            if hasattr(command, "commands") and any(command.commands):
                # it is a group
                em.add_field(
                    name="Sub-Commands",
                    value="".join(
        ...
        em = discord.Embed(title="Cogs list (Not Commands!)")
    ...
    async def cmd_settings(
        self, ctx: Context, entry: str = "", *, val: str = ""
    ):
    ...
    @commands.guild_only()
    @commands.command(name="newsettings")
    async def settings(self, ctx: Context, *, stuff: str = ""):
        if not stuff:  # blank
            # should show all settings
            pass  # TODO: wrds.json IMPL
        args = stuff.split()  # by space
        if len(args) == 1:
            pass
    ...
    async def info(self, ctx: commands.Context):
        if ctx.invoked_subcommand is None:
            ...
            await ctx.send(
                ...
                .add_field(
                    name=self.bot.user, value=f"ver `{botinfo['version']}`"
                )
                .add_field(name="Member count", value=len(ctx.guild.members))
                .set_footer(
                    text=f"Run {ctx.prefix}`help info` for more info about the info command (Sounds funny)"
                )
            )

    @info.command(
        name="user", help="info about a user (can be outside of this server!)"
    )
    async def info_user(
        ...
        if user.discriminator:
            other_desc += (
                f":warning: username has conflict: {user.discriminator}\n"
            )
        await ctx.send(
            embed=discord.Embed(
    ...
    async def info_member(self, ctx, member: discord.Member = None):
        ...
        if member.discriminator:
            other_desc += (
                f":warning: username has conflict: {member.discriminator}\n"
            )
        embed = discord.Embed(
        ...
        if member.is_on_mobile():
            embed.add_field(name="Device", value="Mobile").add_field(
                name="Mobile Status", value=getsStatus(member.mobile_status)
            )
        else:
            embed.add_field(name="Device", value="Desktop / Web App")
        embed.add_field(
            name="Desktop Status", value=getsStatus(member.desktop_status)
        ).add_field(
            name="Web App Status", value=getsStatus(member.web_status)
        ).set_author(
            name=member, icon_url=member.avatar_url
        ).add_field(
            name="Nickname",
            value=member.nick or f"<{member.mention} has no nickname>",
        ).add_field(
            name="Roles",
            value=", ".join([r.mention for r in member.roles[1:]]) or "<None>",
        ).set_footer(
            text="Member joined at", icon_url=ctx.guild.icon_url
        )
    ...
    async def info_channel(self, ctx, channel: discord.TextChannel = None):
        ...
        embed = (
            discord.Embed(
                title=f"Text Channel {channel.name}",
                description=f"```{channel.id}```{other_desc}",
            ...
            .add_field(
                ...
                else channel.category.name,
            )
            .add_field(
                name="Mention",
                value=f"{channel.mention} / `{channel.mention}`",
            )
        ...
        if any(await channel.invites()):
            embed.add_field(
                name="Invites",
                value=", ".join(
                    f"[{invite.id}]({invite.url})"
                    async for invite in channel.invites()
                ),
            )
        await ctx.send(embed=embed)
    ...
    @info.command(
        name="server",
        help="info about the current server",
        aliases=["guild", "srv", "g"],
    )
    @commands.guild_only()
    async def info_server(self, ctx):
        ...
        embed = (
            discord.Embed(
                title="Server info",
                description=ctx.guild.description or "<description not set>",
            ...
            .add_field(
                name="Categories count", value=len(ctx.guild.categories)
            )
            .add_field(
                name="Sudoers",
                value=", ".join(
                    [
                        ...
                        for r in settings[f"g{ctx.guild.id}"]["sudoers"]
                    ]
                )
                or "<None>",
            )
            ...
            .add_field(
                name="System channel",
                value=ctx.guild.system_channel.mention
                if ctx.guild.system_channel
                else "Not set",
            )
            .add_field(
                name="Region", value=str(ctx.guild.region) or "Not set / found"
            )
            .add_field(
                name="afk",
                value=f"{ctx.guild.afk_timeout or '<no afk timeout>'} sec / {ctx.guild.afk_channel.name if ctx.guild.afk_channel else '<no afk channel>'}",
            )
            .add_field(
                name="Public updates channel",
                value=ctx.guild.public_updates_channel.mention
                if ctx.guild.public_updates_channel
                else "Not a public server / not set",
            )
    ...
    @info.command(
        name="bot",
        help="info about this discord bot",
        aliases=["merlin", "this", "self"],
    )
